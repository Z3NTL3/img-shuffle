...
// to multiple consumers. But we didn't want that. That would mean when the handler runs in parallel with several clients, they all would see the same random image.
//
